function McEChartsParallel(){
    //___mcUtils参考mc-utils.js
    this.mcUtils = ___mcUtils;
    this.param = null;
    this.domClass = "mc-echarts-parallel";
}
/*务必带此render方法：否则组件不会被追踪渲染*/
McEChartsParallel.prototype.render = function(param){
...
McEChartsParallel.prototype.parallel = function () {
    var _t = this;
...
McEChartsParallel.prototype.getTitle = function(echartsDom){
...
McEChartsParallel.prototype.setOption = function(option,param,data){
    if(data.code!=0){
        return;
    }
    var dataList = data.data;
    //拿到数据 我们转换一下 变成echarts的数据
    option.parallelAxis = dataList[0].x;
    option.series.data = dataList[1].y;
    param.myEcharts.setOption(option);
    param.myEcharts.resize();
}
/*把组件安装一下*/
___mcUtils.install({component:new McEChartsParallel()})
