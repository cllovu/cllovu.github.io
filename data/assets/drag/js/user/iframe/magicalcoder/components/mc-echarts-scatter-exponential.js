function McEChartsScatterExponential(){
    //___mcUtils参考mc-utils.js
    this.mcUtils = ___mcUtils;
    this.param = null;
    this.domClass = "mc-echarts-scatter-exponential";
}
/*务必带此render方法：否则组件不会被追踪渲染*/
McEChartsScatterExponential.prototype.render = function(param){
...
McEChartsScatterExponential.prototype.scatter = function () {
    var _t = this;
...
McEChartsScatterExponential.prototype.setOption = function(option,param,data){
    ...
    for(var j=0;j<dataList.length;j++){
        var item = dataList[j];
        seriesData.push([Number(item[xFieldName]),Number(item[yFieldName])]);
    }
    // debugger
...
/*把组件安装一下*/
___mcUtils.install({component:new McEChartsScatterExponential()})
