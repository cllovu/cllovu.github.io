...
IframeUi.prototype.fixDynamicDomAfterRender = function(param){
    ...
    /*智能记住上一次聚焦的tabs标签*/
    $(".magicalcoder-tab-title").click(function(){
        if(typeof iframeUi != 'undefined'){
            var api = iframeUi.magicalApi;
            setTimeout(function(){
                var focusNode = api.getFocusNode();
                if(focusNode.magicalCoder.identifier=='magicalcoder-tab-title'){
                    var parentNode = api.findParentNode({id:focusNode.magicalCoder.id});
                    ...
                    if(children && children.length>0){
                        for(var i=0;i<children.length;i++){
                            ...
                            var arr = [];
                            if(clazz){
                                var newArr = clazz.split(/\s+/);
                                for(var j=0;j<newArr.length;j++){
                                    if(newArr[j]!=='layui-this'){
                                         arr.push(newArr[j])
                                    }
                                }
                            }
                            if(focusNode.magicalCoder.id===node.magicalCoder.id){
                                arr.push("layui-this");
                                index = i;
                            }
                            node.attributes['class'] = arr.join(" ");
                        }
                    }
                    var tabContentNode = api.findNextNode({id:parentNode.magicalCoder.id})
                    var itemChildren = tabContentNode.magicalCoder.children;
                    if(itemChildren && itemChildren.length>0){
                        for(var i=0;i<itemChildren.length;i++){
                            var node = itemChildren[i];
                            var clazz = node.attributes['class'];
                            var arr = [];
                            if(clazz){
                                var newArr = clazz.split(/\s+/);
                                for(var j=0;j<newArr.length;j++){
                                    if(newArr[j]!=='layui-show'){
                                         arr.push(newArr[j])
                                    }
                                }
                            }
...
IframeUi.prototype.download = function(html){
    var source = [
        ...
        '<script type="text/javascript" src="assets/drag/js/user/iframe/layui/2.5.4/components/import.js"></script>'
       ]
    //设置样式
    var style = [];
