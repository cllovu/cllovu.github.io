function McEChartsSankey(){
    //___mcUtils参考mc-utils.js
    this.mcUtils = ___mcUtils;
    this.param = null;
    this.domClass = "mc-echarts-sankey";
}
/*务必带此render方法：否则组件不会被追踪渲染*/
McEChartsSankey.prototype.render = function(param){
...
McEChartsSankey.prototype.sankey = function () {
    var _t = this;
...
McEChartsSankey.prototype.getTitle = function(echartsDom){
...
McEChartsSankey.prototype.setOption = function(option,param,data){
    if(data.code!=0){
        return;
    }
    var dataList = data.data;
    //拿到数据 我们转换一下 变成echarts的数据
    option.series.data = dataList[0].x;
    option.series.links = dataList[1].y;
    param.myEcharts.setOption(option);
    param.myEcharts.resize();
}
/*把组件安装一下*/
___mcUtils.install({component:new McEChartsSankey()})
