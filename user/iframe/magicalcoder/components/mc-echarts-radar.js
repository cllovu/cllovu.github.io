function McEChartsLine () {
  ...
  this.param = null;
  this.domClass = "mc-echarts-radar";
}
...
McEChartsLine.prototype.radar = function () {
  var _t = this;
...
McEChartsLine.prototype.setOption = function (option, param, data) {
  ...
  var xAxisData = [];//indicator数据
  var seriesData = [];//y数据
  var dataValue = []
  for (var j = 0; j < dataList.length; j++) {
    var item = dataList[j];
    xAxisData.push({ name: item.name, max: item.max });
    seriesData.push(item.x)
  }
  dataValue.push(seriesData)
  option.radar.indicator = xAxisData;
